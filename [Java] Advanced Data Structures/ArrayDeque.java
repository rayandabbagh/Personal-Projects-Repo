...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        ...
        if (size == backingArray.length) {
            Object[] updatedArray = new Object[backingArray.length * 2];
            for (int j = 0; j < backingArray.length; j++) {
                updatedArray[j + 1] = backingArray[mod(front + j, size)];
            }
    ...
    public void addLast(T data) {
        ...
        if (size == backingArray.length) {
            Object[] updatedArray = new Object[backingArray.length * 2];
            for (int j = 0; j < backingArray.length; j++) {
                updatedArray[j] = backingArray[mod(front + j, size)];
            }
            backingArray = (T[]) updatedArray;
            front = 0;
        }
    ...
    public T removeFirst() {
        if (size == 0) {
            throw new NoSuchElementException("Since the queue is empty, we can't remove anything");
        }
